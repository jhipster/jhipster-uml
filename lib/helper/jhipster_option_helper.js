...
exports.askForService = function(classes) {
  ...
  while (shouldContinueAsking) {
    ...
    var service = askForServiceType();
    classesToTreat.forEach(function(element) {
      listService[element] = service;
    });
    require('inquirer').prompt([
        {
          type: 'confirm',
  ...
  require('inquirer').prompt([
      {
        type: 'list',
        name: 'answer',
        message: 'Please choose the service type:',
        choices: choicesList,
        filter: function(val) {
          return val;
        }
