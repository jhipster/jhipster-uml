...
EntitiesCreator.prototype.createEntities = function() {
  ...
  for (var i in Object.keys(this.parsedData.classes)){
    if (Object.keys(this.parsedData.classes).hasOwnProperty(i)) {
       ...
       */
      if(this.parsedData.classes[classId].name.toLowerCase() === this.USER.toLowerCase()){
        console.warn(
          chalk.yellow(
  ...
  for(var i in Object.keys(this.parsedData.classes)){
    if (Object.keys(this.parsedData.classes).hasOwnProperty(i)) {
      ...
      initializedEntity = this.setDTO(
        initializedEntity,
        this.parsedData.classes[classId].name);
...
EntitiesCreator.prototype.setFieldsOfEntity = function(classId) {
  for(var i = 0; i < this.parsedData.classes[classId].fields.length; i++) {
    var fieldId = this.parsedData.classes[classId].fields[i];

    var field = {
      fieldId: this.entities[classId].fields.length+1,
      fieldName: lowerCase(this.parsedData.fields[fieldId].name)
    };

    if (this.parsedData.types[this.parsedData.fields[fieldId].type]) {
      field.fieldType = this.parsedData.types[this.parsedData.fields[fieldId].type].name;
    } else if (this.parsedData.enums[this.parsedData.fields[fieldId].type]) {
      field.fieldType = this.parsedData.enums[this.parsedData.fields[fieldId].type].name;
      field.fieldValues = this.parsedData.enums[this.parsedData.fields[fieldId].type].values.join(',');
    }
...
EntitiesCreator.prototype.setValidationsOfField = function(field, fieldId, classId) {
  ...
  for(var j in this.parsedData.fields[fieldId].validations){
    if (this.parsedData.fields[fieldId].validations.hasOwnProperty(j)) {
      var validationName= j;
      field.fieldValidateRules.push(validationName);

      if(validationName !== "required"){
        field["fieldValidateRules"+_s.capitalize(validationName)] = this.parsedData.fields[fieldId].validations[j];
      }
    }
  }
};
...
EntitiesCreator.prototype.setRelationshipOfEntity = function(classId) {
 for(var k in this.parsedData.classes[classId].injectedFields) {
    if (this.parsedData.classes[classId].injectedFields.hasOwnProperty(k)) {
      var injectId = this.parsedData.classes[classId].injectedFields[k];
      ...
      var relationshipName = this.splitField(this.parsedData.injectedFields[injectId].name).relationshipName;
      ...
      var relationshipOwnerSide = {
        ...
        otherEntityName: lowerCase(this.parsedData.classes[this.parsedData.injectedFields[injectId].type].name),
        relationshipType: this.parsedData.injectedFields[injectId].cardinality
      };

      // fill the relationship on the other side of the association
      // according to the relationType
      var relationshipOtherSide = {
        relationshipId : this.entities[this.parsedData.injectedFields[injectId].type].relationships.length + 1,
        relationshipName: this.getAssociationFieldName(this.parsedData.injectedFields[injectId].association, lowerCase),
        relationshipFieldName: this.getAssociationFieldName(this.parsedData.injectedFields[injectId].association, lowerCase),
        otherEntityName: lowerCase(this.parsedData.classes[this.parsedData.associations[this.parsedData.injectedFields[injectId].association].type].name)
      };
      ...
      switch(relationshipOwnerSide.relationshipType) {
        case cardinalities.ONE_TO_ONE:
          ...
          relationshipOwnerSide.otherEntityField = this.splitField(this.parsedData.injectedFields[injectId].name).otherEntityField;
        ...
        case cardinalities.ONE_TO_MANY:
          ...
          relationshipOtherSide.otherEntityField = this.getOtherEntityField(this.parsedData.injectedFields[injectId].association);//By default set at 'id'
      ...
      this.entities[this.parsedData.injectedFields[injectId].type].relationships.push(relationshipOtherSide);
    }
  }
};
...
EntitiesCreator.prototype.writeJSON = function(classList) {
  ...
  for(var k in this.entities) {
    if (this.entities.hasOwnProperty(k) && classList.indexOf(k)!==-1){
      var file = '.jhipster/'+this.parsedData.classes[k].name+'.json';
 ...
 */
 EntitiesCreator.prototype.getAssociationFieldName = function(associationId , modifCase) {
  if (!this.parsedData.associations[associationId].name
      && !this.parsedData.associations[associationId].type) {
    throw new BidirectionalAssociationUseException(
      'Bidirectional associations are forbidden.');
  }
  var fieldName=this.parsedData.associations[associationId].name;
  ...
  if( fieldName === undefined || fieldName.length === 0 || fieldName === ""){
    fieldName=this.parsedData.classes[this.parsedData.associations[associationId].type].name;
  }
 ...
 EntitiesCreator.prototype.getOtherEntityField = function(associationId){
   return this.splitField(this.parsedData.associations[associationId].name).otherEntityField;
 };
