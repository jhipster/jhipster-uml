...
EntitiesCreator.prototype.setUniDirectionalRelationship =
    function(classId, injectId) {
  console.log('name ' + this.parsedData.getInjectedField(injectId).name);
  console.log(' with ' + this.parsedData.getAssociation(
      this.parsedData.getInjectedField(injectId).association).name);
  var relationship = {
    relationshipId: this.entities[classId].relationships.length + 1,
    relationshipName: this.parsedData.getInjectedField(injectId).name.toLowerCase(),
  ...
  if (relationship.relationshipType === cardinalities.ONE_TO_ONE) {
    relationship.otherEntityRelationshipName =
      this.getAssociationFieldName(
        this.parsedData.getInjectedField(injectId).association,
        lowerCase);
  }
...
EntitiesCreator.prototype.setBirectionalRelationship = function(classId, injectId) {
    ...
    if (this.parsedData.getClass(classId).injectedFields.hasOwnProperty(k)) {
      ...
      if (!this.parsedData.getAssociation(this.parsedData.getInjectedField(injectId).association).name) { // uni-directional relationships
        ...
        if (this.parsedData.getInjectedField(injectId).cardinality !== cardinalities.ONE_TO_MANY) {
          ...
          this.setBirectionalRelationship(classId, injectId);
        }
      } else { // bidirectional
        this.setBirectionalRelationship(classId, injectId);
      }
