...
function setSourceAssociationsForClass(relatedAssociations, classId) {
  for (let i = 0; i < relatedAssociations.from.length; i++) {
    ...
    if (association.isInjectedFieldInFromRequired && association.type === cardinalities.ONE_TO_MANY) {
      console.warn(
        chalk.yellow(
    ...
    if (association.isInjectedFieldInToRequired && association.type === cardinalities.MANY_TO_ONE) {
      console.warn(
        chalk.yellow(
    ...
    if ((association.isInjectedFieldInToRequired || association.isInjectedFieldInFromRequired) && association.type === cardinalities.MANY_TO_MANY) {
      console.warn(
        chalk.yellow(
          `From ${parsedData.getClass(association.from).name} to ${parsedData.getClass(association.to).name}, a Many-to-Many exists and none of its's side can be required. Removing the required flag.`));
      association.isInjectedFieldInToRequired = false;
      association.isInjectedFieldInFromRequired = false;
    }
    if ((association.isInjectedFieldInToRequired || association.isInjectedFieldInFromRequired) && association.type === cardinalities.MANY_TO_MANY) {
