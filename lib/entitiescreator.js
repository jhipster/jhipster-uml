...
function setRelationshipOfEntity(classId) {
  ...
  for (let i = 0; i < associations.from.length; i++) {
    let otherSplitField;
    let splitField;
    let association = parsedData.getAssociation(associations.from[i]);
  ...
  for (let i = 0; i < associations.to.length; i++) {
    let splitField;
    let otherSplitField;
    let association = parsedData.getAssociation(associations.to[i]);
    let relationship = {
