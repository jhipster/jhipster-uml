...
var chalk = require('chalk'),
    ...
    areJHipsterEntitiesEqual = require('./helper/object_helper').areJHipsterEntitiesEqual,
    ...
    BidirectionalAssociationUseException = require('./exceptions/bidirectional_association_use_exception'),
...
EntitiesCreator.prototype.setUnidirectionalRelationship = function(classId, injectId) {
  var relationship = {
    ...
    relationshipName: _s.camelize(this.parsedData.getInjectedField(injectId).name),
    otherEntityName: _s.decapitalize(_s.camelize(this.parsedData.getClass(this.parsedData.getInjectedField(injectId).type).name)),
  ...
  if (relationship.relationshipType === cardinalities.ONE_TO_ONE) {
    ...
    relationship.otherEntityRelationshipName =
      _s.decapitalize(this.getAssociationFieldName(
        this.parsedData.getInjectedField(injectId).association, _s.camelize));
  } else if (relationship.relationshipType === cardinalities.MANY_TO_MANY) {
    relationship.ownerSide = true;
  }
...
EntitiesCreator.prototype.setBidirectionalRelationship = function(classId, injectId) {
  ...
  var relationshipOwnerSide = {
    relationshipId: this.entities[classId].relationships.length + 1,
    relationshipName: _s.decapitalize(_s.camelize(relationshipName)),
    otherEntityName: _s.decapitalize(_s.camelize(this.parsedData.getClass(this.parsedData.getInjectedField(injectId).type).name)),
    relationshipType: this.parsedData.getInjectedField(injectId).cardinality
  };
  ...
  var relationshipOtherSide = {
    ...
    relationshipName: _s.decapitalize(this.getAssociationFieldName(this.parsedData.getInjectedField(injectId).association, _s.camelize)),
    otherEntityName: _s.decapitalize(_s.camelize(this.parsedData.getClass(this.parsedData.getAssociation(this.parsedData.getInjectedField(injectId).association).type).name))
  };
  ...
  switch(relationshipOwnerSide.relationshipType) {
    case cardinalities.ONE_TO_ONE:
      ...
      relationshipOwnerSide.otherEntityRelationshipName = _s.decapitalize(relationshipOtherSide.relationshipName);
      //relationshipOwnerSide.otherEntityRelationshipName = _s.decapitalize(this.parsedData.getInjectedField(injectId).customName);
      relationshipOtherSide.relationshipType = cardinalities.ONE_TO_ONE;

      relationshipOtherSide.ownerSide = false;
      relationshipOtherSide.otherEntityRelationshipName = _s.decapitalize(relationshipOwnerSide.relationshipName);
      //relationshipOtherSide.otherEntityRelationshipName = _s.decapitalize(this.parsedData.getInjectedField(injectId).customName);
      break;
    case cardinalities.ONE_TO_MANY:
      ...
      relationshipOtherSide.otherEntityField = this.getOtherEntityField(this.parsedData.getInjectedField(injectId).association);
      break;
    case cardinalities.MANY_TO_ONE:
      ...
      relationshipOtherSide.otherEntityRelationshipName = _s.decapitalize(relationshipOwnerSide.relationshipName);
      //relationshipOtherSide.otherEntityRelationshipName = _s.decapitalize(this.parsedData.getInjectedField(injectId).customName);
      relationshipOtherSide.relationshipType = cardinalities.ONE_TO_MANY;
      break;
...
EntitiesCreator.prototype.setRelationshipOfEntity = function(classId) {
  for (var k in this.parsedData.getClass(classId).injectedFields) {
    ...
    if (this.parsedData.getAssociation(this.parsedData.getInjectedField(injectId).association).name
        || this.parsedData.getInjectedField(injectId).cardinality === cardinalities.ONE_TO_MANY) {
 ...
 * lowercase if no information about it is available
 *
 */
...
EntitiesCreator.prototype.setService = function(entity, entityName) {
  if (Object.keys(this.listService).indexOf(entityName) !== -1) {
