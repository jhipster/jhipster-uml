...
EntitiesCreator.prototype.setUnidirectionalRelationship = function(classId, injectId) {
  var relationship = {
    ...
    otherEntityName: _s.capitalize(_s.camelize(this.parsedData.getClass(this.parsedData.getInjectedField(injectId).type).name)),
...
EntitiesCreator.prototype.setBidirectionalRelationship = function(classId, injectId) {
  ...
  var relationshipOwnerSide = {
    relationshipId: this.entities[classId].relationships.length + 1,
    relationshipName: _s.camelize(relationshipName),
  ...
  var relationshipOtherSide = {
    ...
    relationshipFieldName: _s.decapitalize(this.getAssociationFieldName(this.parsedData.getInjectedField(injectId).association, _s.camelize)),
    otherEntityName: _s.capitalize(_s.camelize(this.parsedData.getClass(this.parsedData.getAssociation(this.parsedData.getInjectedField(injectId).association).type).name))
  };
  ...
  switch(relationshipOwnerSide.relationshipType) {
    case cardinalities.ONE_TO_ONE:
      ...
      relationshipOtherSide.otherEntityRelationshipName = _s.decapitalize(relationshipOwnerSide.relationshipFieldName);
      break;
    case cardinalities.ONE_TO_MANY:
      relationshipOwnerSide.otherEntityRelationshipName = _s.decapitalize(relationshipOtherSide.relationshipFieldName);
...
EntitiesCreator.prototype.setRelationshipOfEntity = function(classId) {
  for (var k in this.parsedData.getClass(classId).injectedFields) {
    ...
    if (!this.parsedData.getClass(classId).injectedFields.hasOwnProperty(k)
       || this.parsedData.getClass(classId).name.toLowerCase() === this.USER.toLowerCase()) {
      continue;
    }

    var injectId = this.parsedData.getClass(classId).injectedFields[k];

    console.log('>>>>>>>>>>>>>>>>>>>>>>< ' + this.parsedData.getAssociation(this.parsedData.getInjectedField(injectId).association).name);
    console.log('>>>>>>>>>>>>>>>>>>>>>>>>>>  ' + (this.parsedData.getInjectedField(injectId).cardinality === cardinalities.ONE_TO_MANY));
