...
function toJDL(parsedData, options) {
  ...
  if (options) {
    var jdlOptionArray = convertOptions(options, parsedData.classes);
...
function convertOptions(options, parsedClasses) {
  var optionArray = [];
  addDTOOptions(optionArray, options, parsedClasses);
  addPaginationOptions(optionArray, options, parsedClasses);
  addServiceOptions(optionArray, options, parsedClasses);
  addSkipClientOption(optionArray, options, parsedClasses);
  addSkipServerOption(optionArray, options, parsedClasses);
  addMicroserviceOption(optionArray, options, parsedClasses);
  addSearchEngineOptions(optionArray, options, parsedClasses);
  return optionArray;
}

function addDTOOptions(optionArray, options, parsedClasses) {
  if (!options.listDTO) {
    return;
  }
  optionArray.push(new JDLBinaryOption({
    name: BinaryOptions.DTO,
    entityNames: options.listDTO, parsedClasses,
    value: BinaryOptionValues.dto.MAPSTRUCT
  }));
}
function addPaginationOptions(optionArray, options, parsedClasses) {
  ...
  for (let i = 0, paginationTypes = Object.keys(entitiesByPaginationType); i < paginationTypes.length; i++) {
    optionArray.push(new JDLBinaryOption({
      ...
      entityNames: entitiesByPaginationType[paginationTypes[i]], parsedClasses,
