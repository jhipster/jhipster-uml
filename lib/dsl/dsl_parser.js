...
DSLParser.prototype.fillClassesAndFields = function () {
  ...
  addMicroserviceOptions(this.result.microservice,this.parsedData);
};
...
function addMicroserviceOptions(microservices, parsedData) {
  Object.keys(microservices).forEach(function (option) {
    var collection = (microservices[option].list[0] === '*')
        ? Object.keys(parsedData.classes)
        : microservices[option].list;
    if (microservices[option].excluded.length !== 0) {
      collection = collection.filter(function (className) {
        return microservices[option].excluded.indexOf(className) === -1;
      });
    }
    collection.forEach(function (className) {
      parsedData.getClass(className).microserviceName = option;
    });
  });
}
