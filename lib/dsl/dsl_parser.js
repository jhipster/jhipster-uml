...
DSLParser.prototype.fillAssociations = function() {
  ...
  for (var i = 0; i < relationships.length; i++) {
    // we change a many-to-one to a one-to-many
    var relationship = this.changeMtOtoOtM(relationships[i]);
...
DSLParser.prototype.checkEntityDeclaration = function(relationship, associationId){
  ...
  if (!this.parsedData.getClass('User') && (relationship.from.name === 'User' || relationship.to.name === 'User')) {
