...
if (!options.db && !fs.statSync('./.yo-rc.json').isFile()) {
  throw new buildException(
      exceptions.WrongCall,
...
var listDTO = (options.dto) ? jhipsterOptionHelper.askForDTO(parsedData.classes, options.dtoValue): [];
var listPagination = (options.paginate) ? jhipsterOptionHelper.askForPagination(parsedData.classes, options.paginationValue) : {};
var listService = (options.service) ? jhipsterOptionHelper.askForService(parsedData.classes, options.serviceValue) : {};
...
var angularSuffixes = (options.angularSuffix) ? jhipsterOptionHelper.askForAngularSuffixes(parsedData.classes, options.angularSuffixValue) : {};
var microserviceNames = (options.microserviceName) ? jhipsterOptionHelper.askForMicroserviceNames(parsedData.classes, options.microserviceNameValue) : {};
var searchEngines = (options.searchEngine) ? jhipsterOptionHelper.askForSearchEngines(parsedData.classes, options.searchEngineValue) : [];
