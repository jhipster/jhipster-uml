...
function findConstraints() {
  ...
  for (let i = 0; i < root.ownedRule.length; i++) {
    const element = root.ownedRule[i];
...
function handleAttributes(classElement) {
  for (let i = 0; i < classElement.ownedAttribute.length; i++) {
    const attribute = classElement.ownedAttribute[i];
    if (!attribute.$.name) {
      throw new BuildException(
        exceptions.NullPointer,
...
function addClass(classElement) {
  const names = ParserHelper.extractClassName(classElement.$.name);
  const classData = {
...
function completeAssociation(association, element) {
  ...
  case 'composite':
    if (Object.hasOwnProperty.call(association.type, 'MANY_TO_ONE')) {
  ...
  case 'shared':
    if (element.upperValue && element.upperValue[0].$.value === '*'
      && Object.hasOwnProperty.call(association.type, 'MANY_TO_MANY')) {
      association.type = cardinalities.MANY_TO_MANY;
    } else if (!element.upperValue && Object.hasOwnProperty.call(association.type, 'MANY_TO_ONE')
        || (element.upperValue && element.upperValue[0] && element.upperValue[0].$.value !== '*')) {
      association.type = cardinalities.MANY_TO_ONE;
    }
    break;
  default:
...
function addConstraint(constraintElement) {
  ...
  const name = constraintElement.$.name;
  const type = parsedData.getType(parsedData.getField(constraintElement.$.constrainedElement).type);
  const enumType = parsedData.getEnum(parsedData.getField(constraintElement.$.constrainedElement).type);
  ...
  // not nil, but ''
  const value = constraintElement.specification[0].$.value;
