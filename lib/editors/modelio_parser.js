...
ModelioParser.prototype.addType = function(typeName, typeId) {
  ...
  this.parsedData.addType(typeId, typeName);
};
...
ModelioParser.prototype.fillEnums = function() {
  this.rawEnumsIndexes.forEach(function(index) {
    ...
    enumElement.ownedLiteral.forEach(function(literalIndex) {
      ...
      this.parsedData.getEnums()[enumElement.$['xmi:id']].values.push(
...
ModelioParser.prototype.checkForUserClass = function(element) {
  if (!this.parsedData.getUserClassId() && element.$.name.toLowerCase() === 'user') {
    this.parsedData.setUserClassId(element.$['xmi:id']);
  }
};
...
ModelioParser.prototype.addRegularField = function(element, classId) {
  ...
  if (element.$.type) {
    this.parsedData.getFields()[element.$['xmi:id']].type = element.$.type;
  } else {
    ...
    this.parsedData.getFields()[element.$['xmi:id']].type = typeName;
  }
  this.parsedData.getClasses()[classId].fields.push(element.$['xmi:id']);
};
...
ModelioParser.prototype.addInjectedField = function(element, classId) {
  ...
  if (element.upperValue && element.upperValue[0].$.value) {
    this.parsedData.getInjectedFields()[element.$['xmi:id']].isUpperValuePresent =
      element.upperValue[0].$.value === '*';
  }

  this.parsedData.getInjectedFields()[element.$['xmi:id']].cardinality =
    parser_helper.getCardinality(
      this.parsedData.getInjectedFields()[element.$['xmi:id']],
      this.parsedData.getAssociations());
  this.parsedData.getClasses()[classId].injectedFields.push(element.$['xmi:id']);
};
...
ModelioParser.prototype.fillConstraints = function() {
  this.rawValidationRulesIndexes.forEach(function(index) {
    ...
    if (this.parsedData.getFields()[constraint.$.constrainedElement]) {
      previousValidations =
        this.parsedData.getFields()[constraint.$.constrainedElement].validations;
    }

    if (!this.databaseTypes.isValidationSupportedForType(
        this.parsedData.getTypes()[this.parsedData.getFields()[constraint.$.constrainedElement].type],
        name)) {
