...
var cardinalities = require('../cardinalities'),
    NoCardinalityException = require('../exceptions/no_cardinality_exception');
...
exports.isAnId = function(name, className) {
  ...
  return name.length === 2 && /^id$/.test(name.toLowerCase())
    || regex.test(name) || /^id[A-Z].*/.test(name);
};

/**
 * Returns field's cardinality based on the association and the field's
 * attributes.
 * @param {Object} injectedField the field.
 * @param {Object} associations the associations holder.
...
exports.getCardinality = function(injectedField, associations) {
  ...
  } else if (this.isManyToOne(injectedField.isUpperValuePresent,
        associations[injectedField.association].isUpperValuePresent)) {
...
/**
 * Checks whether the relationship is a "many-to-one".
 * @param injectedFieldUpperValuePresence if the UpperValue flag is set in the
 *                                        injected field.
 * @param associationUpperValuePresence if the UpperValue flag is
*                                       set in the association.
 * @returns {boolean} the result.
 */
exports.isManyToOne = function (injectedFieldUpperValuePresence, associationUpperValuePresence) {
  return !injectedFieldUpperValuePresence && associationUpperValuePresence;
};
