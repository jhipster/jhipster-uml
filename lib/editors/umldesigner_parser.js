...
function handleAttributes(element) {
  for (let i = 0; i < element.ownedAttribute.length; i++) {
    ...
    if (!parser_helper.isAnId(attribute.$.name)) {
      //If not a primitiveType and not a Enum (So it's a association)
      if (attribute.$.type && !parsedData.getEnum(attribute.$.type) && !parsedData.getType(attribute.$.type)){
        parseFieldToAssociation(attribute, element.$['xmi:id']);
      } else {
...
function parseFieldToAssociation(attribute, classId) {
  
  var associationElement = {
    $: { 'xmi:id': attribute.$['xmi:id'] },
    ownedEnd: []
  };
  var ownerEndFrom = {
    $: { 
    ...
    lowerValue: [{$: {value: attribute.lowerValue[0].$.value}}],
    upperValue: [{$: {value: attribute.upperValue[0].$.value}}]
  }
  associationElement.ownedEnd.push(ownerEndFrom);
  var ownerEndTo = {
    $: { 
      name: _.lowerFirst(parsedData.getClass(classId).name) + _.upperFirst(attribute.$.name),
      type: classId,
    },
    lowerValue: [{$: {value: '0'}}],
    upperValue: [{$: {value: '*'}}]
  }
  associationElement.ownedEnd.push(ownerEndTo);
  associationElement.ownedComment = attribute.ownedComment;
  
  addAssociation(associationElement);
