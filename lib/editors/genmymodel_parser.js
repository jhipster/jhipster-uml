...
GenMyModelParser.prototype.addClass = function (element) {
  ...
  if (isReservedClassName(classData.name)) {
    throw new buildException(
        exceptions.IllegalName,
        `The passed name '${classData.name}' is reserved.`);
  }
  if (isReservedTableName(classData.tableName, this.databaseTypes.getName())) {
    throw new buildException(
        exceptions.IllegalName,
        `The passed table name '${classData.tableName}' is reserved.`);
...
GenMyModelParser.prototype.addRegularField = function (element, classId) {
  if (isReservedFieldName(element.$.name, this.databaseTypes.getName())) {
    throw new buildException(
        exceptions.IllegalName,
        `The passed field name '${element.$.name}' is reserved.`);
  }
